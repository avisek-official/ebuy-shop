...
export const WomenCloth = () => {
  document.title = "Buy Women's clothes online at eBuy";
  const dispatch = useDispatch();

  const products = useSelector((state) => [state.allProductReducer]);
  const productItems = products[0];
  let womenClothingItems = productItems.filter(
  ...
  return (
    ...
    >
      ...
      >
        <span>Total No. of items: {womenClothingItems.length}</span>
      ...
      {womenClothingItems.length === 0 && <SkeletonLoader />}
      {womenClothingItems.length !== 0 && (
        <div
